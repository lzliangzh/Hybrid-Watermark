...
- Align encrypt & decrypt method with Gaussian Shading / RingID. Current <=> Gaussian_Shading </=> Gaussian_Shading_chacha / RingID: 
  - `m = ((sd + self.gs_key) % 2).cpu().numpy()` (`watermark_hybrid.py`: 191)
  - `reversed_sd = (reversed_m + self.gs_key) % 2` (`watermark_hybrid.py`: 294)
  - no encrypt or decrypt in `_generate_candidate_database`
- Implement MaXSive & ...
